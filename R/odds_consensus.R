...
odds_consensus<- function(week_num, num_games){
  games<- c(1:(num_games))
  url_list<- purrr::map(.x= '', paste, "body > div.dialog-off-canvas-main-canvas > div.layout-container > main > div.layout-content > div > article > div > div.layout--max-width-1088px.layout.layout--onecol > div > div.block.block-oddsshark-data-blocks.block-consensus-block > div > div > div.picks-table-wrap.picks-count-", games, " > div.pick-table-content > table",
                        sep = "")
